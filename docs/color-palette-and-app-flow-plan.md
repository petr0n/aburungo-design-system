...
**Status:** all phases complete (0–5), **re-verified against the tree 2026-08-20** by `pnpm verify:plan` — 34/34. That run is what found 5.6 recorded as done while three call sites still rendered as bare text · **Decisions here are provisional — see below** · **Scope:** `aburungo-design-system` only · **Palette:** v3 "Zuihoden", in hand and reviewed · **Branch prefix:** `feat/tokens-*`, `feat/flow-*`
...
| 1.7 | **Repo-wide hex sweep of `preview/`, not just the two swatch pages.** The swatch pages (`03-color-brand`, `06-color-semantic-fg`) print hex as literal text; four more hardcode colour values that no token change reaches — `16-buttons-accent.html` (`color:#fff` on the CTA), `13-elevation.html` (`background:#fff`), `24-progress.html` (`var(--surface-2,#f4f4f5)` — a *cool-grey* fallback), `11-spacing-scale.html`. **These are the dangerous ones:** v3's card ground is `#FFFDF8` and its wells are warm stone, so every `#fff` and `#f4f4f5` goes visibly wrong the moment the warm palette lands, while the page still looks fine today | `grep -riE "#[0-9a-f]{3,6}" preview/` returns only the swatch pages' `<code>` samples, and those read from computed styles. **Re-run 2026-08-20:** two survivors in `28-progress-contrast.html` — `#33685e`, a hardcoded duplicate of `--color-link`, and `#4a3714`, which matched no token at all and is now `--ogon-900`. Both are the failure this task exists to prevent: change the palette and that page silently keeps the old colour. **`preview/_sandbox/` is deliberately out of scope** — those are one-off diagnostic renders, and a before/after comparison needs literal colours to compare *against*; they are not spec pages and nothing reads them for values |
| 1.8 | Strip hex values out of prose (`README.md`, `SKILL.md`, `CLAUDE.md`, `colors_and_type.css` header) — refer to roles, and point at `docs/colors.md` for values | no hex literal appears in any `.md` that *specifies* a colour; `grep -rn "aa3bff"` returns only this plan's own record of the sweep. **Reworded 2026-08-20** — as originally written the criterion could never pass, because this row quotes the string it bans |
...
> Verified 2026-08-15, **corrected 2026-08-20.** Done: 5.1 the palette is v3, 5.2 assets copied, 5.3 `brand.css` merged, 5.4 `Button` wears `bg-action`, 5.5 the nine repainted, 5.5b inverse chrome on both `AppHeader` and `KanaKeyboard`, 5.5c/5.5d closed above, 5.7/5.8 the gate and lint run green, 5.11 `docs/colors.md` is the v3 document.
>
> **5.6 was recorded as done here and was not.** The variant was dropped from `Button`; three call sites were left behind, rendering as bare text on three landing screens. Closed 2026-08-20 — see the row. The lesson is not about that variant: **this note was written by reading the diff rather than the screens.** A dropped variant leaves no error anywhere, so only a render or a typechecker can see it, and the three survivors were all in untyped mirrors.
>
...
| 5.6 | ~~Migrate the deprecated `Button` `accent` variant to `primary` and drop it~~ — **half-done until 2026-08-20.** The variant was dropped from `Button` on schedule; **three call sites were not migrated** and nobody noticed for five days, because a `variant` a component does not implement is not an error. `variants[variant]` returns `undefined`, the class list comes out empty, and the button renders as **bare text** — measured `background: rgba(0,0,0,0)`, `border-width: 0px` on all three. They were `ui_kits/app/screens.jsx:191` and `ui_kits/desktop-screens.jsx:223,253`, all of them the landing CTA. Two more of the same bug were found and fixed in `ui_kits/mobile/` in #35. Every one was in a **hand-written JSX mirror**; the typechecked harnesses never had it | **done 2026-08-20** — 0 call sites, and all three now measure Ai-iro `#1F3A66` on `#FFFDF8` |
