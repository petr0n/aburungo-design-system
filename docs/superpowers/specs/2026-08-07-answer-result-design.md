...
| Reveal style | **Quiet reveal with a small mark.** Neutral ground, no full-bleed green/red wash; the ○/✕ carries the outcome. |
| Verdict prose | **None.** The mark is the signal. This is what keeps the component inside the assessment rule, which still bans verdict wording. |
| Glyph ownership | One `Maru` component. `AnswerResult` uses it internally; `KanaPracticePage` uses it directly on tiles. The glyph is defined once. |
...
`AnswerResult` renders a `bg-surface-2` rounded block containing the mark, the children, and — when `userAnswer` is passed — a muted `you typed: …` line. No coloured wash and no verdict sentence.

`Maru` renders ○ in Rokushō (`text-success-500`) for `recalled`, ✕ in Akane (`text-error-500`) for `review`.

## Accessibility

Phase 3's gate requires meaning on three channels and states that a screen reader must never receive a bare "circle". With no visible verdict word, the third channel is a visually-hidden `sr-only` span carrying the approved wording — `recalled` / `worth another look`. Sighted users get shape plus colour; screen-reader users get a word. The gate is satisfied literally, so no amendment to it is needed.

Shape alone distinguishes the two states, so the component does not fail for a colourblind user if colour is stripped.
