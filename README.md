...
```bash
pnpm build        # brand check → lint → tsup (ESM + .d.ts) → regenerate tokens
pnpm dev          # tsup --watch for live rebuilds
pnpm typecheck    # tsc --noEmit
pnpm lint         # oxlint (adherence) + no raw hex / non-DS fonts in components
...
**Token values live in exactly one file.** `src/tokens.css` is the source; `pnpm build`
propagates it to `dist/tokens.plain.css` (which the 27 preview pages import) and
regenerates the `@theme` block inside the storybook and all three UI kits, between
`build-tokens` markers. Editing between those markers is pointless — the next build
overwrites it.
...
| `src/index.css` | Package entry — `@import "tailwindcss"` + `@import "./tokens.css"` + `@font-face` + base resets. Not for app import. |
...
| `dist/` | Compiled output (ESM + `.d.ts`) — generated by `pnpm build`, not committed. The one exception is `dist/tokens.plain.css`, which **is** committed: the preview pages import it and are served with no build step |
...
> The `zinc-*`, `green-*`, `red-*` and `--accent` names this section used to cite were retired when the token sources were collapsed. Use `surface`, `surface-2`, `border`, `border-strong`, `fg`, `fg-muted`, `fg-subtle`, `fg-faint`, and the `success-*` / `error-*` pairs.
...
- Standard pattern: `hover:bg-surface-2 active:bg-surface-2` on neutral buttons, `active:` press states on primary buttons. No transforms, no shrink, no glow.
- Disabled = `opacity-50` (or `opacity-40` on a heavy button).
- Focus: a 2px ring in the focus role — the one place the accent appears on a neutral input.

### Borders
- Default `1px solid` in `border`. Stronger `border-strong` on form inputs and secondary buttons to give them more presence.
- Inputs darken to `fg-subtle` on focus (plus the focus ring).

### Shadows
- `shadow-card` (a single hairline drop) on the review card, `shadow-key` on kana keys. That's the entire elevation system.
...
### Cards
- Rounded‑2xl, `bg` background, `border` 1px, `shadow-card`. That's the formula. Always.
